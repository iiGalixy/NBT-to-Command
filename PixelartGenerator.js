      ...
      setTimeout(() => {
        ...
        if (pixelColorRGBA.a != 0) {
          ...
          if (blockNameAndMeta.startsWith("sand") || blockNameAndMeta.startsWith("gravel") || blockNameAndMeta.includes("powder")) {
              appender.append(`setblock\ ~${x - _halfW}\ ~\ ~${y - _halfH}\\tstone`);
            commandsUsed++;
          }

          appender.append(`setblock\ ~${x - _halfW}\ ~1\ ~${y - _halfH}\ ${blockNameAndMeta}`);
          commandsUsed++;
        }
        ...
        if (isLastOne) {
          ...
          appender.append(`say ${commandsUsed} commands are used.. Took ${secondsTook} seconds to make.. (${config.buildFromCenter ? "Builds from the center!" : "Builds from the left top corner!"})`, true);
